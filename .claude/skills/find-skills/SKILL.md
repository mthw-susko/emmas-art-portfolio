...
3. If the user agrees, run:
   ```bash
   /opt/homebrew/bin/caliber skills --query "<relevant terms>"
   ```
...
5. Install the selected skills:
   ```bash
   /opt/homebrew/bin/caliber skills --install <slug1>,<slug2>
   ```
...
-> If yes: run `/opt/homebrew/bin/caliber skills --query "react frontend"`
-> Show the user the results, ask which to install
-> Run `/opt/homebrew/bin/caliber skills --install <selected-slugs>`
...
-> If yes: run `/opt/homebrew/bin/caliber skills --query "docker deployment"`

User: "I need to write tests for this Python ML pipeline"
-> "Would you like me to find skills for Python ML testing?"
-> If yes: run `/opt/homebrew/bin/caliber skills --query "python machine-learning testing"`
